...
import com.ahmadda.infra.jwt.exception.JwtPropertiesException;
...
public class JwtProperties {
    ...
    private void validateProperties(final String secretKey, final Duration accessExpiration) {
        if (secretKey == null || secretKey.isBlank()) {
            throw new JwtPropertiesException("JWT 시크릿 키가 비어있습니다.");
        }
        if (accessExpiration == null) {
            throw new JwtPropertiesException("JWT 액세스 만료 시간이 지정되지 않았습니다.");
        }
        if (accessExpiration.isNegative() || accessExpiration.isZero()) {
            throw new JwtPropertiesException("JWT 액세스 만료 시간은 0보다 커야 합니다.");
        }
    }
}
